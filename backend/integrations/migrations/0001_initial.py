...
def migrate_existing_google_data(apps, schema_editor):
    ...
    for usuario in Usuario.objects.exclude(google_sub__isnull=True).exclude(
        google_sub=""
    ):
        account, _ = GoogleAccount.objects.update_or_create(
            usuario_id=usuario.pk,
            defaults={
                "google_user_id": usuario.google_sub,
                "email": usuario.email,
        ...
        GoogleCalendar.objects.get_or_create(
            ...
            defaults={
                ...
                "primary": getattr(settings, "GOOGLE_CALENDAR_ID", "primary")
                == "primary",
                "enabled": True,
            },
        )
    ...
    if GoogleAccount.objects.count() == 1:
        calendar = GoogleCalendar.objects.filter(
            account=GoogleAccount.objects.first()
        ).first()
        if calendar:
            for evento in Evento.objects.exclude(google_event_id__isnull=True).exclude(
                google_event_id=""
            ):
    ...
    operations = [
        ...
        migrations.CreateModel(
            ...
            fields=[
                ...
                ("remote_sequence", models.IntegerField(blank=True, null=True)),
                ...
                (
                    ...
                    models.ForeignKey(
                        ...
                        to="integrations.googlecalendar",
                    ),
                ),
                (
                    "evento",
